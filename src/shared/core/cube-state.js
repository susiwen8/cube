import { getLayerCoordinate, parseMove } from './moves.js';

export const FACE_DEFINITIONS = Object.freeze({
...
export const FACE_NAMES = Object.freeze(Object.keys(FACE_DEFINITIONS));
...
export function getStaticSlotMetadata(size = 3) {
...
export function getMovePermutation(size, move) {
  const notation = parseMove(move).notation;
...
function buildPermutation(size, move) {
  ...
  const layer = getLayerCoordinate(size, parsed);
